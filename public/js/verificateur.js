const verificateur = {
    ...
    async verifyLot(lotId) {
        if (!lotId) return window.showToast("Lot ID est vide", "warning");

        window.showToast(`Vérification du lot ${lotId}...`, "info");
        const lot = await database.getLot(lotId);
        if (!lot) {
            window.showToast("Lot introuvable dans le registre", "error");
            return;
        }
        
        const transfers = await database.getTransfersByLot(lotId);
        this.renderResult(lot, transfers);
        window.showToast("Authenticité confirmée ✓", "success");
    },
